...
def load(data_dir):
    """Read logs and video-side features, returning a dict split into the official partitions."""
    vid2author = {}
...
def encode(splits):
    """Map categorical features to consecutive IDs. Unseen values are mapped to the UNK slot for that field.
    Return (X, y, users) per split, where X is int32 (N, len(FIELDS)), together with field_dims."""
    tr = splits['train']
    ...
    unk = [len(v) for v in vocabs]                 # Reserve an UNK slot at the end of each field
